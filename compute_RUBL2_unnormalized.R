...
Nseq = as.numeric(args[2])
germline_field = args[3]
files = args[4:(3+(length(args)-3)/2)]
...
library('shm',lib.loc=package_path) # deprecated, install shazam instead
...
IgUniFrac2 = function(otu.tab, tree) {
...
for (i in 1:length(vjl_groups)) {
  ...
  ## Germlines based on IMGT are aligned from the same start position, but lengths may slightly differ.
  ...
  #rownames(dm) = colnames(dm) = alnd_seqs
  DM = as.dist(dm)
  ...
  if (length(DM)!=0) {
    ...
    uf = IgUniFrac2(incidence_table, seq_tree)       
...
diag(RUBL) = 0
RUBL_unnorm = sum_unshared_branchlength
diag(RUBL_unnorm) = 0

### Write results:
outfile = paste("RUBL2.seed-",seed_value,".Nseq-",Nseq,".",germline_field,".tab",sep="")
write.table(as.data.frame(RUBL),file=outfile)
outfile2 = paste("RUBL2.unnormalized.seed-",seed_value,".Nseq-",Nseq,".",germline_field,".tab",sep="")
write.table(as.data.frame(RUBL_unnorm),file=outfile2)
